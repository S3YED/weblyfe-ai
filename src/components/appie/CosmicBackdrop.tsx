...
export default function CosmicBackdrop({ intensity = 'medium' }: Props) {
  const goldOpacity = intensity === 'low' ? 0.07 : intensity === 'high' ? 0.18 : 0.12;
  const greenOpacity = intensity === 'low' ? 0.18 : intensity === 'high' ? 0.32 : 0.24;
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden bg-[#001711]">
      <motion.div
        className="absolute -right-40 -top-40 h-[60vmax] w-[60vmax] rounded-full blur-3xl"
        style={{ background: `radial-gradient(circle, rgba(253,211,138,${goldOpacity}) 0%, transparent 65%)` }}
        animate={{ x: [0, 40, -20, 0], y: [0, -30, 20, 0] }}
        transition={{ duration: 28, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute -bottom-60 -left-40 h-[55vmax] w-[55vmax] rounded-full blur-3xl"
        style={{ background: `radial-gradient(circle, rgba(37,62,54,${greenOpacity}) 0%, transparent 65%)` }}
        animate={{ x: [0, -30, 20, 0], y: [0, 30, -10, 0] }}
        transition={{ duration: 34, repeat: Infinity, ease: 'easeInOut' }}
      />
      {/* Tactical grid: peripheral HUD decoration, very faint. */}
      <div className="hud-grid absolute inset-0 opacity-[0.5]" />
      <div
        className="absolute inset-0 opacity-[0.04]"
        style={{
          backgroundImage: 'radial-gradient(circle at 50% 50%, #fdd38a 1px, transparent 1px)',
