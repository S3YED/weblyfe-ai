...
// Sent right after the Instant Appie subscription is created. Warmer than the
// magic-link email: it congratulates, sets the voicenotes-first expectation, and
// the CTA is the one-time setup link (a magic link) that drops them straight
// into the dashboard.
...
export async function sendWelcomeEmail(
  ...
  if (!apiKey) {
    // eslint-disable-next-line no-console
    console.log(`[brevo:dev-stub] would welcome ${args.toEmail}: ${args.setupUrl}`);
    return { ok: true, messageId: 'dev-stub' };
  }
  ...
  const body = {
    sender: { email: 'seyed@weblyfe.ai', name: 'Appie · Weblyfe' },
    to: [{ email: args.toEmail, name: args.toName ?? args.toEmail }],
    subject: 'Welkom bij Instant Appie, laten we hem opzetten',
    htmlContent: renderWelcomeHtml(args),
    textContent: renderWelcomeText(args),
  };
...
function renderWelcomeText(args: SendWelcomeArgs): string {
  return `Welkom bij Instant Appie.

Je betaling is gelukt en je persoonlijke Techwiz staat klaar. Nog een laatste stap: zet hem op via de knop hieronder. Dat kost je twee minuten.

${args.setupUrl}

Het mooiste? Je hoeft niets te typen. Stuur Appie gewoon een voicenote in Telegram, dan pakt hij het op. Inbox triagen, leads kwalificeren, afspraken plannen. Jij praat, hij werkt.

De link werkt 15 minuten en kan een keer gebruikt worden. Lukt het niet? Reply op deze mail, we lezen mee.

Tot zo,
Appie · Weblyfe.ai`;
}

function renderWelcomeHtml(args: SendWelcomeArgs): string {
  const safeUrl = args.setupUrl;
...
<title>Welkom bij Instant Appie</title>
</head>
<body style="margin:0;padding:0;background:#020e0a;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#F6FEFC;">
  <span style="display:none !important;visibility:hidden;mso-hide:all;font-size:1px;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">
    Je persoonlijke Techwiz staat klaar. Nog een laatste stap om hem op te zetten.
  </span>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background:#031D16;border-radius:24px;overflow:hidden;border:1px solid rgba(223,183,113,0.18);">
          ...
          <tr>
            <td style="padding:24px 40px 8px 40px;">
              <h1 style="margin:0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:34px;line-height:1.12;font-weight:700;color:#F6FEFC;letter-spacing:-0.4px;">
                Welkom.<br/>Je Appie staat klaar.
              </h1>
            </td>
          </tr>
          <tr>
            <td style="padding:20px 40px 0 40px;">
              <p style="margin:0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:16px;line-height:1.55;color:#c8d4cf;">
                Je betaling is gelukt. Nog een laatste stap: zet je persoonlijke Techwiz op. Dat kost je twee minuten.
              </p>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:28px 40px 0 40px;">
              <div style="background:#020e0a;border:1px solid rgba(223,183,113,0.14);border-radius:14px;padding:20px 22px;">
                <p style="margin:0 0 6px 0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:2px;color:#DFB771;text-transform:uppercase;font-weight:600;">
                  Het mooiste
                </p>
                <p style="margin:0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:15px;line-height:1.55;color:#c8d4cf;">
                  Je hoeft niets te typen. Stuur Appie gewoon een <strong style="color:#F6FEFC;">voicenote</strong> in Telegram, dan pakt hij het op. Inbox triagen, leads kwalificeren, afspraken plannen. Jij praat, hij werkt.
                </p>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:24px 40px 40px 40px;">
              <p style="margin:0 0 6px 0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:12px;line-height:1.55;color:#7a8983;">
                De link werkt 15 minuten en kan een keer gebruikt worden. Vragen? Reply op deze mail, we lezen mee.
              </p>
